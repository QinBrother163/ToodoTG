	...
	for(var k=0;k<3;++k){
		...
		if(k==2){
			for(var i=0;i<document.getElementById('contain-div').getElementsByTagName('div').length;++i){
					if(image_index<10){
						items[k][i]=(document.getElementById('contain-20'+image_index));
					}else{
						items[k][i]=(document.getElementById('contain-2'+image_index));
					}
					image_index++;
			}
		}
...
document.onkeydown=function(event){
	...
	if(e && e.keyCode==keyMap.ok){
		switch(index){
			...
			case 2:
                if(current==0){
                    window.location.href = 'merchantsDessert.html';
                }
			break;
		}
	}
